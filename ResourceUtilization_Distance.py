...
def experiment_distance():
    ...
    for dist_avg in distances:
        print(f"\n=== Distance={dist_avg} ===")
        # 构造统一距离矩阵
        node_distance = np.full((device_count, device_count), dist_avg, dtype=float)
        np.fill_diagonal(node_distance, 0)
        ...
        for run in range(repeat_times):
            ...
            for ep in range(dqn_cfg['episodes']):
                ...
                while not done:
                    ...
                    agent.update()
                    s = s2
            util2 = env.compute_during / env.completion_time if env.completion_time > 0 else 0
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('distance_utilization_stats.png', dpi=150)
    plt.show()

    # --- 保存数据 ---
    np.save('util_distance_iptu.npy', dict(util_iptu))
    np.save('util_distance_dqn.npy', dict(util_dqn))
    with open('utilization_distance_comparison.csv', 'w', newline='') as f:
        writer = csv.writer(f)
